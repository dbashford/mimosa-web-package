'use strict';
var copyDirSyncRecursive, exec, fs, hogan, isReallyWindows, langs, logger, moduleConfig, path, registration, rimraf, _, __generateConfigText, __runNPMInstall, __writeApplicationStarter, __writeConfig, _package;
...
registration = function(config, register) {
  ...
  if (config.isPackage) {
    ...
    if (process.platform === "win32") {
      return exec('uname', function(error, stdout, stderr) {
        if (!error) {
...
__runNPMInstall = function(config, next) {
  ...
  return exec("npm install", function(err, sout, serr) {
    var altOutputTarFile, done, outputTarFile, pack, tarCommand, tarballName;
...
__writeApplicationStarter = function(config) {
  var appJsInPath, appJsOutPath, appJsText, chosenLang, err, ext, lang, level, pack, prepend, prependLang, rootPathFromAppjs, serverExtension, serverRelPath, _fn, _i, _len, _ref, _ref1;
